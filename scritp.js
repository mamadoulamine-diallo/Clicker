document.addEventListener("DOMContentLoaded", () => {
  ...
  const unlocks = [
    ...
    {
      score: 30,
      message: "🛠️ Tu sais utiliser JavaScript pour modifier la page",
    },
    { score: 50, message: "🐙 Git est ton ami pour versionner ton code" },
    {
      score: 75,
      message: "🌐 Tu connais la différence entre frontend et backend",
    },
  ...
  function checkUnlocks() {
    unlocks.forEach((unlock) => {
      if (counter >= unlock.score && !unlocked.includes(unlock.message)) {
        ...
        if (unlock.score === 10) {
          showLevelBanner("Niveau CSS débloqué !");
  ...
  function updateShopDisplay() {
    document.getElementById(
      "itemClavier"
    ).textContent = `🧠 ${shopItems.clavier.name} x${shopItems.clavier.quantity} (+${shopItems.clavier.power}/clic) — ${shopItems.clavier.cost} pts`;

    document.getElementById(
      "itemStagiaire"
    ).textContent = `⏳ ${shopItems.stagiaire.name} x${shopItems.stagiaire.quantity} (+${shopItems.stagiaire.power}/sec) — ${shopItems.stagiaire.cost} pts`;
  }
...
function showLevelBanner(message) {
  ...
  setTimeout(() => banner.classList.add("show"), 100); // petit délai d'apparition

  setTimeout(() => {
    banner.classList.remove("show");
    setTimeout(() => {
      banner.classList.add("hidden");
    }, 500); // attendre que l'opacité revienne à 0 avant de cacher
  }, 3000); // visible 3s
}
